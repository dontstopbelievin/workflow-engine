              ...
              <div class="row">
                <div class="col-md-3">
                  @if (Auth::user()->role->name === 'Заявитель')
                    <a href="{{ url('applications/create', ['process' => $process]) }}" class="btn btn-info mb-3">Создать Заявку</a>
                    <form action="{{ url('applications/search')}}" method="POST">
                        @csrf
                    ...
                    <table class="table">
                      ...
                      @foreach($arrayApps as $app)
                          <tr>
                              ...
                              @endif
                                  <td>
                                  <button class="rounded-circle bg-white" onclick="window.location='{{url('applications/view', ['process_id' => $process["id"] , 'application_id' => $app["id"]])}}'">
