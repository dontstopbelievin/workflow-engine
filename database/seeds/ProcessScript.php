...
class ProcessScript extends Seeder
{
    ...
    public function run()
    {
        ...
        $this->create_postanovka_na_o4ered_zu();
        $this->create_prodaja_zu_edinovremenno();
        $this->create_predostavlenie_ish_materialov();
    }
    ...
    public function create_postutilizacia_obektov(){
        ...
        $request->replace(['template_state' => 1, 'table_name' => 'p15_postutilizaciya', 'process_id' => $process->id, 'template_doc_id' => $template_doc->id, 'role_id' => $role2->id, 'order' => 2, 'to_citizen' => 1]);
        app('App\Http\Controllers\TemplateController')->store($request);
    }

    public function create_postanovka_na_o4ered_zu(){
        //Постановка на очередь на получение земельного участка
        $process = Process::where('name', 'Постановка на очередь на получение земельного участка')->first();
        //org name
        $org = CityManagement::where('name', 'Управление архитектуры, градостроительства и земельных отношений города Нур-Султан')->first();
        $process->main_organization_id = $org->id;
        $process->need_map = 1;
        ...
        //'parent_role_id' => $request->parent_role_id,
        $role2 = Role::where('name', 'Руководитель управления архитектуры, градостроительства и земельных отношений города Нур-Султан')->first();
        $process->roles()->attach($role2->id, [
            'can_reject' => 1,
            'can_send_to_revision' => 0,
            'can_ecp_sign' => 1,
        ...
        $role3 = Role::where('name', 'Специалист отдела заключения договоров и контроля за их использованием, подготовки решений об изъятии и резервировании земель')->first();
        $process->roles()->attach($role3->id, [
            'can_reject' => 0,
            'can_send_to_revision' => 0,
            'can_ecp_sign' => 0,
        ...
        $role4 = Role::where('name', 'Руководитель отдела заключения договоров и контроля за их использованием, подготовки решений об изъятии и резервировании земель')->first();
        $process->roles()->attach($role4->id, [
            'can_reject' => 1,
            'can_send_to_revision' => 1,
            'can_ecp_sign' => 1,
        ...
        //final doc template
        $request = new \Illuminate\Http\Request();
        $template_doc = TemplateDoc::where('name', 'Шаблон постановка в очередь на ЗУ')->first();
        $request->replace(['template_state' => 1, 'table_name' => 'p14_uvedomlenie_postanovke_u4et', 'process_id' => $process->id, 'template_doc_id' => $template_doc->id, 'role_id' => $role3->id, 'order' => 3, 'to_citizen' => 1]);
        app('App\Http\Controllers\TemplateController')->store($request);
    }

    public function create_prodaja_zu_edinovremenno(){
        //Продажа земельного участка в частную собственность единовременно либо в рассрочку
        $process = Process::where('name', 'Продажа земельного участка в частную собственность единовременно либо в рассрочку')->first();
        //org name
        $org = CityManagement::where('name', 'Управление архитектуры, градостроительства и земельных отношений города Нур-Султан')->first();
        $process->main_organization_id = $org->id;
        $process->need_map = 1;
        ...
        $role3 = Role::where('name', 'Руководитель отдела организации работы земельной комиссии')->first();
        $process->roles()->attach($role3->id, [
            'can_reject' => 0,
            'can_send_to_revision' => 0,
            'can_ecp_sign' => 0,
        ...
        $role6 = Role::where('name', 'Руководитель отдела организации работы земельной комиссии')->first();
        $process->roles()->attach($role6->id, [
            'can_reject' => 0,
            'can_send_to_revision' => 1,
            'can_ecp_sign' => 1,
        ...
        $role7 = Role::where('name', 'Руководитель управления архитектуры, градостроительства и земельных отношений города Нур-Султан')->first();
        $process->roles()->attach($role7->id, [
            'can_reject' => 1,
            'can_send_to_revision' => 0,
            'can_ecp_sign' => 1,
            'can_motiv_otkaz' => 0,
        ...
        $role8 = Role::where('name', 'Гос Правовой отдел акимата')->first();
        $process->roles()->attach($role8->id, [
            'can_reject' => 1,
            'can_send_to_revision' => 0,
            'can_ecp_sign' => 1,
            'can_motiv_otkaz' => 0,
        ...
        $role9 = Role::where('name', 'Заместитель акима')->first();
        $process->roles()->attach($role9->id, [
            'can_reject' => 1,
            'can_send_to_revision' => 0,
            'can_ecp_sign' => 1,
            'can_motiv_otkaz' => 0,
    ...
    public function create_predostavlenie_ish_materialov(){
        // 	Предоставление исходных материалов при разработке проектов строительства и реконструкции
        $process = Process::where('name', 'Предоставление исходных материалов при разработке проектов строительства и реконструкции')->first();
        //org name
        $org = CityManagement::where('name', 'Управление архитектуры, градостроительства и земельных отношений города Нур-Султан')->first();
        $process->main_organization_id = $org->id;
        $process->need_map = 1;
